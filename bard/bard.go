...
func NewBard(sessionID, proxy string, ck []string) *Bard {
...
func (b *Bard) generateRequestBody(message string, options Options) (url.Values, error) {
	...
	//	return nil, err
	//}
	fmt.Println([]string{string(fReqBytes)})
...
func (b *Bard) SendMessage(message string, options Options) (*ResponseBody, error) {
	params := fmt.Sprintf("?bl=%s&_reqid=%d&rt=%s", "boq_assistant-bard-web-server_20230523.13_p0", b.RequestID, "c")
	fmt.Println(params)
